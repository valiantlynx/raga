...
RUN DEPLOY_TARGET=node yarn turbo run build --filter=raga --concurrency 2
...
COPY --from=installer /app/apps/raga/package.json .
COPY --from=installer /app/apps/raga/build /app/build
